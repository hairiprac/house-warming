...
export function MenuNavs({ variant = "Dot", className }: MenuNavsProps) {
  ...
  const icons = {
    Up: <ChevronUp size={20} color="#255435" />,
    Down: <ChevronDown size={20} color="#255435" />,
    Add: <Plus size={20} color="#255435" />,
    Close: <X size={20} color="#255435" />,
    Subtract: <Minus size={20} color="#255435" />,
    Right: <ChevronRight size={20} color="#255435" />,
  }

  return (
    <div className={cn("relative size-8 flex items-center justify-center", className)}>
      {icons[variant]}
    </div>
  )
}
