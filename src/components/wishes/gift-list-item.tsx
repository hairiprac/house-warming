      ...
      <div className="flex flex-1 flex-col gap-3 items-start min-w-0">
        <div className="flex flex-col gap-1 items-start w-full">
          ...
          <div className="flex gap-3 items-center w-full">
            <p className={`text-[20px] font-bold text-[#255435] leading-normal whitespace-nowrap ${dimmed ? "opacity-50" : ""}`}>
              {item.Item}
            </p>
          ...
          {item.ActualName && item.ActualName !== "-" && (
            <p className={`text-[16px] font-semibold text-[#255435] leading-normal truncate w-full ${dimmed ? "opacity-50" : ""}`}>
              {item.ActualName}
            </p>
          )}
          {item.Option && item.Option !== "-" && (
            <p className={`text-[14px] font-semibold text-[#255435] leading-normal ${dimmed ? "opacity-50" : ""}`}>
              {item.Option}
            </p>
          )}
          <div className={`flex gap-1.5 items-center text-[14px] font-semibold whitespace-nowrap ${dimmed ? "opacity-50" : ""}`}>
            {item.Price && (
        ...
        {item.Desc && (
          <p className={`text-[12px] font-semibold text-[#255435] leading-normal w-full ${dimmed ? "opacity-50" : ""}`}>
            {item.Desc}
          </p>
        )}
      </div>
